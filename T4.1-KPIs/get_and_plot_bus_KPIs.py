...
def load_bus_edges(net_file):
    ...
    for edge in root.findall("edge"):
        ...
        for lane in lanes:
            ...
            if allow:
                allowed.update(allow.split())
            if disallow:
                disallowed.update(disallow.split())

        bus_allowed = (
            "bus" in allowed or
            ("all" in allowed and "bus" not in disallowed) or
            (not allowed and "bus" not in disallowed)
        )
        bus_exclusive = allowed == {"bus"}
        if bus_allowed:
...
def plot_bus_routes(net, bus_edges, exclusive_bus_edges, filename):
    ...
    ax.legend(handles=legend_patches, loc="lower left", fontsize=9, framealpha=0.8)
